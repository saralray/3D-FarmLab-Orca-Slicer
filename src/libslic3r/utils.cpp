...
{
	// Use the canonical app key ("OrcaSlicer") rather than the branded display
	// name (SLIC3R_APP_NAME, e.g. "3D-FarmLab-Orca-Slicer") for the
	// machine-readable "; generated by <name>" file headers. Downstream tooling
	// identifies the slicer by this name; in particular Moonraker only extracts a
	// G-code file's embedded thumbnails when it recognizes the slicer here, so a
	// branded fork name is treated as an unknown slicer and Klipper printers
	// (e.g. the Snapmaker U1) show no print preview. The UI branding is
	// unaffected — that still uses SLIC3R_APP_NAME.
	return std::string(SLIC3R_APP_KEY " " SoftFever_VERSION);
}
